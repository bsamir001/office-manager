...
class ReservedAppointmentsListView(APIView):
    permission_classes = [IsAuthenticated]
...
class DayAppointmentsView(APIView):
    ...
    def post(self, request, appointment_date):
        ...
        user_patents = patent.objects.filter(user=user)

        if request.data.get('selected_patient'):
            selected_patent_id = request.data.get('selected_patient')
            #appointment.patent = get_object_or_404(patent, id=selected_patent_id, user=user)
        else:
            return Response({'message': 'You must select a patient'}, status=status.HTTP_400_BAD_REQUEST)

        payment_url = reverse('turn:pay_api', args=[appointment.id])
...
class UserToPatientView(APIView):

    def get(self, requset):
        user = requset.user
        user_patents = patent.objects.filter(user=user)
        return Response({'user_patents': [patent.id for patent in user_patents]}, status=status.HTTP_200_OK)
